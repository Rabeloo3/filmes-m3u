...
// ==========================================================
// SOLUÇÃO MIXED CONTENT: Redirecionador Automático de Streams
// ==========================================================
    ...
    if (currentTab === 'live') {
        ...
        items.forEach(item => {
            ...
            btn.onclick = () => {
                ...
                const rawStream = `${creds.server}/live/${creds.user}/${creds.pass}/${item.stream_id}.m3u8`;
                launchStream(proxyStreamUrl(rawStream));
            };
            grid.appendChild(btn);
        });
    } else {
        grid.className = 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 p-4';
        items.forEach(item => {
            const btn = document.createElement('button'); btn.type = 'button';
            const posterImg = item.stream_icon || item.cover || defLogo; 
            
            btn.className = 'channel-item poster-btn group relative flex flex-col bg-[#0d1527] border border-slate-800/80 rounded-xl overflow-hidden cursor-pointer text-left shadow-lg h-full';
            ...
            btn.innerHTML = `
                <div class="w-full aspect-[2/3] bg-slate-950 flex items-center justify-center relative overflow-hidden shrink-0">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent opacity-0 group-hover:opacity-90 transition-opacity flex items-end p-3">
                        <span class="text-xs font-bold text-cyan-400"><i class="fa-solid fa-play mr-1"></i> Reproduzir</span>
                    </div>
                </div>
                <div class="p-3 flex items-center w-full bg-[#0d1527] flex-grow border-t border-white/[0.02]">
                    <p class="text-xs font-bold text-slate-200 line-clamp-2 leading-tight" title="${item.name}">${item.name}</p>
                </div>
            `;
            btn.onclick = () => {
                if (currentTab === 'vod') {
                    const rawStream = `${creds.server}/movie/${creds.user}/${creds.pass}/${item.stream_id}.${item.container_extension || 'mp4'}`;
                    launchStream(proxyStreamUrl(rawStream));
                }
...
function renderEpisodes(sNum) {
    ...
    (seriesCachedData[sNum] || []).forEach(ep => {
        ...
        btn.onclick = () => { 
            closeSeriesModal(); 
            const rawStream = `${creds.server}/series/${creds.user}/${creds.pass}/${ep.id}.${ep.container_extension || 'mp4'}`;
            launchStream(proxyStreamUrl(rawStream)); 
        };
        box.appendChild(btn);
    });
